...
const HorizontalCard: React.FC<HorizontalCardProps> = React.memo(({
  ...
  return (
    <div className={cardClasses} role="region" aria-label={`${cardtitle} statistics`}>
      <div className="card-body">
        <div className="card-header-section">
          <h6 className="pt-2 mb-2 fw-semibold" id={`card-title-${cardtitle.replace(/\s+/g, '-').toLowerCase()}`}>
            {cardtitle}
          </h6>
          <hr className="my-2" />
        </div>
        ...
        <div className="d-flex align-items-center mt-2">
          ...
          <div className="ps-3 flex-grow-1">
            <h4 
              ...
              aria-describedby={`card-title-${cardtitle.replace(/\s+/g, '-').toLowerCase()}`}
            >
              {formattedBody}
            </h4>
