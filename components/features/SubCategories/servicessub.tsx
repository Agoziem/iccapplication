...
import { ServiceCategory, ServiceSubCategory } from "@/types/items";
...
/**
 * Enhanced ServicesSub component with comprehensive error handling and type safety
 * Manages service subcategories with create, update, and delete functionality
 * Optimized with React.memo and proper TypeScript typing
 */
const ServicesSubCatForm: React.FC = React.memo(() => {
  const [currentCategory, setCurrentCategory] =
    useState<ServiceCategory | null>(null);
  const [item, setItem] = useState<ServiceSubCategory | null>(null);
  ...
  const { data: subcategories, isLoading: loadingsubcategories } =
    useServiceSubCategories(currentCategory?.id || 0);
  ...
  // Mutations
  const { mutateAsync: createSubCategory } = useCreateServiceSubCategory();
  const { mutateAsync: updateSubCategory } = useUpdateServiceSubCategory();
  const { mutateAsync: deleteSubCategory } = useDeleteServiceSubCategory();
  ...
  // Handle delete modal
  const handleDeleteModal = useCallback((subcategory: ServiceSubCategory) => {
    setItem(subcategory);
    setModal(true);
  }, []);
  ...
  return (
    <div className="card p-4">
      <h6 className="mb-3">Create Service Sub-Category</h6>
      {alert.show && (
        <div className="mb-3">
      ...
      <form onSubmit={handleItem}>
        ...
        <div className="d-flex align-items-center mb-3">
          <input
            ...
            aria-label="Service subcategory name"
          />
          <button
            type="submit"
          ...
          {safeSubcategories.map((subcategory, i) => (
            ...
            >
              <span
                ...
                aria-label={`Edit service subcategory: ${subcategory.subcategory}`}
              >
              ...
              <TiTimes
                ...
                aria-label={`Delete service subcategory: ${subcategory.subcategory}`}
              />
            </div>
          ))}
        </div>
      ...
      <Modal showmodal={modal} toggleModal={closeModal}>
        <p>
          Are you sure you want to delete this service subcategory under{" "}
...
ServicesSubCatForm.displayName = "ServicesSubCatForm";

export default ServicesSubCatForm;
