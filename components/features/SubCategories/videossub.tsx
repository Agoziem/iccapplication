...
/**
 * Enhanced VideosSub component with comprehensive error handling and type safety
 * Manages video subcategories with create, update, and delete functionality
 * Optimized with React.memo and proper TypeScript typing
 */
const VideoSubCatForm: React.FC = React.memo(() => {
  const [currentCategory, setCurrentCategory] = useState<VideoCategory | null>(null);
  const [item, setItem] = useState<VideoSubCategory | null>(null);
  ...
  const { data: subcategories, isLoading: loadingsubcategories } =
    useVideoSubCategories(currentCategory?.id || 0);
  ...
  // Mutations
  const { mutateAsync: createSubCategory } = useCreateVideoSubCategory();
  const { mutateAsync: updateSubCategory } = useUpdateVideoSubCategory();
  const { mutateAsync: deleteSubCategory } = useDeleteVideoSubCategory();
  ...
  const handleEditSubcategory = useCallback((subcategory: VideoSubCategory) => {
    setItem(subcategory);
    setEdit(true);
  }, []);

  // Handle delete modal
  const handleDeleteModal = useCallback((subcategory: VideoSubCategory) => {
    setItem(subcategory);
    setModal(true);
  }, []);
  ...
  return (
    <div className="card p-4">
      <h6 className="mb-3">Create Video Sub-Category</h6>
      {alert.show && (
        <div className="mb-3">
      ...
      <form onSubmit={handleItem}>
            ...
            {loadingCategories ? (
              ...
              <>
                ...
                {safeCategories.map((category) => (
                  <option key={category.id || category.category} value={category.category}>
                    {category.category}
                  </option>
                ))}
              </>
        ...
        <div className="d-flex align-items-center mb-3">
          <input
            ...
            placeholder="Video Subcategory Name"
            required
            disabled={!currentCategory || loading}
            aria-label="Video subcategory name"
          />
          <button 
            type="submit"
          ...
          {safeSubcategories.map((subcategory, i) => (
            ...
            >
              <span
                ...
                aria-label={`Edit video subcategory: ${subcategory.subcategory}`}
              >
              ...
              <TiTimes
                ...
                aria-label={`Delete video subcategory: ${subcategory.subcategory}`}
              />
            </div>
          ))}
        </div>
      ...
      <Modal showmodal={modal} toggleModal={closeModal}>
        <p>
          Are you sure you want to delete this video subcategory under{" "}
...
VideoSubCatForm.displayName = 'VideoSubCatForm';

export default VideoSubCatForm;