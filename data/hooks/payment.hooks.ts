...
export const PAYMENT_KEYS = {
  ...
  listsbyUser: (userId: number) =>
    [...PAYMENT_KEYS.all, "listbyUser", userId] as const,
  list: (organizationId: number) =>
    [...PAYMENT_KEYS.lists(), organizationId] as const,
...
export const fetchPayment = async (
  ...
  const response = await AxiosInstanceWithToken.get(
    `${paymentsAPIendpoint}/payment/${paymentId}/`
  );
  return response.data;
};

export const fetchPaymentByRef = async (
  ref: string
): Promise<PaymentResponse> => {
  const response = await AxiosInstanceWithToken.get(
    `${paymentsAPIendpoint}/payment/reference/${ref}/`
  );
  return response.data;
};
...
export const deletePayment = async (paymentId: number): Promise<void> => {
  await AxiosInstanceWithToken.delete(
    `${paymentsAPIendpoint}/deletepayment/${paymentId}/`
  );
};
...
export const getPaymentsByUser = async (
  ...
  const response = await AxiosInstanceWithToken.get(
    `${paymentsAPIendpoint}/paymentsbyuser/${userId}/`
  );
  return response.data;
};
...
export const usePayments = (
  organizationId: number
): UseQueryResult<PaymentArray, Error> => {
  return useQuery({
...
export const useOrderReport = (
  organizationId: number
): UseQueryResult<PaymentStats, Error> => {
  return useQuery({
...
export const usePaymentbyRef = (
  ref: string
): UseQueryResult<PaymentResponse, Error> => {
  return useQuery({
    queryKey: ["payment", "byRef", ref],
    queryFn: () => fetchPaymentByRef(ref),
    enabled: !!ref,
    onError: (error: Error) => {
...
export const useCreatePayment = (): UseMutationResult<
  PaymentResponse,
  Error,
  { organizationId: number; paymentData: CreatePayment }
> => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ organizationId, paymentData }) =>
      createPayment(organizationId, paymentData),
...
export const useUpdatePayment = (): UseMutationResult<
  PaymentResponse,
  Error,
  { paymentId: number; updateData: UpdatePayment }
> => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ paymentId, updateData }) =>
...
> => {
  ...
  return useMutation({
    ...
    onSuccess: ({ reference, customer, organization }) => {
      if (customer && customer.id) {
        queryClient.invalidateQueries(PAYMENT_KEYS.listsbyUser(customer.id));
      }
      if (organization && organization.id) {
        queryClient.invalidateQueries(PAYMENT_KEYS.list(organization.id));
      }
      queryClient.invalidateQueries(["payment", "byRef", reference]);
    },
    onError: (error: Error) => {
