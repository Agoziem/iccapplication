...
const CustomersPage: FC = () => {
  const [items, setItems] = useState<Customers | null>(null);
  const [showModal, setShowModal] = useState<boolean>(false);
  const [customerID, setCustomerID] = useState<string | null>(null);

  // fetch Order Report
  const { data: orderReport } = useGetOrderReport() as {
    data: OrderReport | undefined;
  };

  useEffect(() => {
    setItems(orderReport?.customers || null);
  }, [orderReport]);
  ...
  const {
    ...
    error,
  } = useFetchUser(`${authAPIendpoint}/getuser/${customerID}`, customerID) as {
    data: User | undefined;
    isLoading: boolean;
    error: any;
  };
