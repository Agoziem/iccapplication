...
const Department: FC<DepartmentPageProps> = ({ params }) => {
  const { id } = params;
  const [department, setDepartment] = useState<DepartmentType | null>(null);
  const [otherDepartments, setOtherDepartments] = useState<DepartmentType[]>(
    []
  );
  ...
  useEffect(() => {
    if (id && depts?.results && depts.results.length > 0) {
      fetchdepartment();
    }
  }, [id, depts]);
    ...
    <section className="px-5 pt-3">
      ...
      <div className="row p-md-4">
        <div className="col-12 col-md-6 ps-0">
          {department?.img_url && (
            <img
              src={department.img_url || "/ICC dept image.png"}
                ...
                {department.staff_in_charge.img_url ? (
                  ...
                  >
                    <img
                      ...
                      alt={department.staff_in_charge.name || "Staff"}
      ...
      <div className="row px-0 px-md-5 text-center">
        {otherDepartments && otherDepartments.length > 0 ? (
          otherDepartments.map((dept, index) => (
            <div key={dept.id || index} className="col-md-6 col-lg-4 mb-4">
